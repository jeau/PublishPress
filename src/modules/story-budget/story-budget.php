...
class PP_Story_Budget extends PP_Module
{
    ...
    public function print_term($term)
    {
        ...
        ?>
    <div class="postbox<?php if (!empty($posts)) {
    echo ' postbox-has-posts';
}
        ?>">
        <div class="handlediv" title="<?php _e('Click to toggle', 'publishpress');
        ?>"><br /></div>
        <h3 class='hndle'><span><?php echo esc_html($term->name);
        ?></span></h3>
        <div class="inside">
            <?php if (!empty($posts)) : ?>
        ...
        ?>" class="manage-column column-<?php echo esc_attr(sanitize_key($key));
        ...
        ?>
            ...
            <?php else: ?>
            <div class="message info"><p><?php _e('There are no posts for this term in the range or filter specified.', 'publishpress');
        ?></p></div>
            <?php endif;
        ?>
        </div>
